...
public partial class AccountManagementPage : ContentPage
{
    ...
    private async void OnSendVerificationClicked(object sender, EventArgs e)
    {
        ...
        await DisplayAlert("Gửi mã xác minh", $"Mã xác minh đã được gửi đến {AppContext.Instance.CurrentUser?.Email}", "Đã hiểu");
    }
    ...
    private async void OnVerifyEmailClicked(object sender, EventArgs e)
    {
        ...
        {
            await DisplayAlert("Lỗi", "Mã xác minh không đúng. Vui lòng thử lại.", "Đã hiểu");
            return;
        }

        if (VerificationCodeEntry != null)
        {
